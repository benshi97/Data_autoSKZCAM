...
\caption{\label{tab:eads_dft_ensemble_errors_mgo}For the monomers on the MgO(001) surface, we estimate the errors for using the revPBE-D4 geometry and $E_\textrm{rlx}$ in the final $E_\textrm{ads}$ of the autoSKZCAM protocol using an ensemble of 6 different DFAs. The errors are calculated as the difference between the true $E_\textrm{ads}^\textrm{true}$ (using the appropriate DFA) and the approximated $E_\textrm{ads}^\textrm{approx}$ using the revPBE-D4 geometry and $E_\textrm{rlx}$.} \\
...
\cline{1-18}
$\epsilon_\textrm{geom}$ & RMSE & 19 & 34 & 20 & 100 & 20 & 46 & 91 & 42 & 45 & 25 & 42 & 73 & 85 & 49 & 53 & 30 \\
\cline{1-18}
\end{longtable}